"""
Morizon parser — uses their internal search API + JSON-LD fallback.
"""
import random
import re
import json
...
def _try_api(session) -> list:
    """Try Morizon's internal search API."""
    results = []
    try:
        api_url = "https://www.morizon.pl/api/search"
        params = {
            "type": "rent",
            "category": "apartment",
            "city": "warszawa",
        ...
        print(f"[Morizon] API status: {r.status_code}")
...
def parse_morizon() -> list:
    ...
    if api_results:
        add(api_results)
        print(f"[Morizon] API: {len(api_results)} listings")
        return results
    ...
    for page in range(1, 6):
        ...
        try:
            ...
            if page_results:
                ...
                print(f"[Morizon] Page {page}: 0. Snippet: {html[:200].replace(chr(10),' ')}")
