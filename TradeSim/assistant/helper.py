...
class TradingAssistant:
    ...
    def chat(
        ...
        if any(w in msg for w in ("пополн", "deposit", "закинуть", "внести", "депозит")):
            return (
                "💵 Пополнение:\n"
                "• «+ Пополнить» — paper USDT\n"
                "• Testnet/Live: faucet или адрес в панели «Биржа»\n"
                "• «Синхр. USDT → paper» — подтянуть баланс биржи боту\n"
                "• История — вкладки «Депозиты» и «Кошелёк»"
            )

        if any(w in msg for w in ("вывод", "вывести", "withdraw", "снять")):
            return (
                "💸 Вывод USDT:\n"
                "• «− Вывести» в шапке\n"
                "• Paper — с виртуального счёта\n"
                "• Биржа — реальный вывод (Testnet/Live, нужен адрес)\n"
                "• Лимит Live вывода в .env: EXCHANGE_MAX_WITHDRAW_USD"
            )
        ...
        if any(w in msg for w in ("учится", "обуча", "сам учит", "реально учит", "нейросет", "фейк", "показух")):
            return (
                ...
                "• 12 агентов = правила Python + ключевые слова\n"
                "• Paper по умолчанию — testnet через BINANCE_API_KEY\n\n"
        ...
        if any(w in msg for w in ("готов", "readiness", "оценк", "идея", "насколько")):
            return (
                ...
                "• Testnet/live через EXCHANGE_ENABLED\n\n"
        ...
        if any(w in msg for w in ("лаборатор", "shadow", "клон", "mock", "мок", "тенев")):
            ...
            return (
                ...
                f"• Те же цены, разные параметры по типу стратегии (DCA/Grid/Momentum/RSI/Scalper)\n"
